...
export default function CartProvider({children}) {
    ...
    useEffect(()=>{
        ...
        localStorage.setItem(
            CART_KEY,JSON.stringify({items:cartItems,totalCount,totalPrice})
        );
    },[cartItems]);
    ...
    const sum=items=>{
        return items.reduce((preValue, curValve)=> preValue+curValve, 0);
    };
    ...
    const changeQuantity=(cartItem, newQuantity)=>{
        const {food}= cartItem;
        const changedCardItem={
            ...cartItem,
            quantity:newQuantity,
            price:food.price*newQuantity,
        };
        setCartItems(
            cartItems.map(item=>(item.food.id === food.id ? changedCardItem:item))
        );
    };
